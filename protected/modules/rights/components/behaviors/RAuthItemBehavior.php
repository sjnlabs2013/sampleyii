...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	...
	{
		return CHtml::linkButton('<i class="fa fa-times"></i>  '.Rights::t('core', 'Delete'), array(
	...
	*/
	...
	{
                return CHtml::linkButton('<i class="fa fa-times"></i>  '.Rights::t('core', 'Remove'), array(
	...
	*/
	...
	{
		return CHtml::linkButton('<i class="fa fa-times"></i> '.Rights::t('core', 'Revoke'), array(
	...
	*/
	public function getRevokePermissionLink(CAuthItem $role)
	{
		$csrf = Rights::getDataCsrf();
                $img = CHtml::image(Yii::app()->baseUrl.'/images/checked.png', '', array('style'=>'height:16px'));

                
		return CHtml::link($img, '#', array(
                        'title'=>Rights::t('core', 'Revoke'),
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
